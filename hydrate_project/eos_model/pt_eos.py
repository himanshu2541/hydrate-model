...
class PTEOS(EquationOfState):
    ...
    def _get_eos_params_and_Z(self, T, P):
        """
        Centralized method to calculate mixing parameters and solve the PT cubic.
        """
        n = len(self.gases)
        ...
        for i, gas in enumerate(self.gases):
            ...
            # Solve for Omega_b analytically (smallest positive root of the PT cubic)
            ...
            Omega_a = 3.0*(zeta_c**2) + 3.0*(1.0 - 2.0*zeta_c)*Omega_b + (Omega_b**2) + 1.0 - 3.0*zeta_c

            # PT alpha function parameter
        ...
        return Z, A, B, C, am, bm, cm, ai, bi, ci, a_mix_matrix
    ...
    def calc_fugacities(self, T, P) -> tuple[dict, np.ndarray]:
        ...
        if P < 1.0:
            return {gas: P * self.y[i] for i, gas in enumerate(self.gases)}, np.ones(len(self.gases))

        Z, A, B, C, am, bm, cm, ai, bi, ci, a_mix_matrix = self._get_eos_params_and_Z(T, P)
        
        phi = np.zeros(len(self.gases))
        f_dict = {}

        # The Q-term is the denominator discriminant for the PT logarithmic integral
        Q = np.sqrt(B**2 + C**2 + 6.0 * B * C)
        
        for i, gas in enumerate(self.gases):
            sum_yaj = np.sum(self.y * a_mix_matrix[i, :])
            
            # Standard Patel-Teja fugacity coefficient formula
            term1 = (bi[i] / bm) * (Z - 1.0)
            term2 = np.log(Z - B)
            
            # The logarithmic integration expansion for PT
            log_term = np.log((Z + 0.5 * (B + C + Q)) / (Z + 0.5 * (B + C - Q)))
            
            # Note: This is the simplified multi-component derivative standard in hydrate modeling
