...
class SRKEOS(EquationOfState):
    ...
    def _get_eos_params_and_Z(self, T, P):
        """
        Centralized method to calculate mixing parameters and solve the SRK cubic.
        ...
        for i, gas in enumerate(self.gases):
            ...
            # For H2/Quantum gases, ensure Database uses Newton's corrected Tc/Pc
            Tr = T / Tc
        ...
        # Solve SRK Z cubic equation: Z^3 - Z^2 + (A - B - B^2)Z - AB = 0
    ...
    def calc_fugacities(self, T, P) -> tuple[dict, np.ndarray]:
        ...
        for i, gas in enumerate(self.gases):
            ...
            # Standard Soave-Redlich-Kwong fugacity coefficient formula
            term1 = (bi[i] / bm) * (Z - 1.0)
            term2 = np.log(Z - B)
            term3 = (A / B) * ((2.0 * sum_yaj / am) - (bi[i] / bm)) * np.log(1.0 + B / Z)
