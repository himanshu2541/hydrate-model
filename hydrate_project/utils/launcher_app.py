...
class LauncherApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        hdr.pack(**P)
        ttk.Label(hdr, text="🔬  Hydrate Equilibrium Model",
                  style="Title.TLabel").pack(anchor=tk.W)
        ttk.Label(hdr, text="Configure inputs, then click  ▶ Run Calculation",
        ...
        ttk.Button(add_row, text="＋  Add gas", style="TButton",
                   command=self._add_gas_row_dialog).pack(side=tk.LEFT)
        ...
        self._liq_display = ttk.Label(disp_row, text="H₂O: 1.0000",
        ...
        ttk.Label(prom_toggle, text="Promoter:", style="Muted.TLabel",
        ...
        prom_cb = ttk.Combobox(prom_toggle, textvariable=self._prom_cb_var,
                               values=prom_opts, state="readonly",
        ...
        ttk.Label(self._prom_frac_frame, text="Mol fraction:",
                  style="Muted.TLabel").pack(side=tk.LEFT)
        prom_ent = ttk.Entry(self._prom_frac_frame, textvariable=self._promoter_frac,
                             width=10, font=("Segoe UI", 10))
        prom_ent.pack(side=tk.LEFT, padx=(6, 10))
        self._prom_hint = ttk.Label(self._prom_frac_frame, text="",
                                    style="Muted.TLabel")
        self._prom_hint.pack(side=tk.LEFT)
        self._prom_frac_frame.pack_forget()   # hidden until promoter selected
        self._promoter_frac.trace_add("write",
                                      lambda *_: self._refresh_liquid_display())
        ...
        for name in ["Peng-Robinson", "Soave-Redlich-Kwong", "Patel-Teja"]:
            ...
            ttk.Checkbutton(eos_row, text=name, variable=v,
                            style="TCheckbutton").pack(side=tk.LEFT, padx=(0, 16))
        ...
        self._cache_label = ttk.Label(cache_top, text="Checking cache…",
                                      style="Muted.TLabel")
        self._cache_label.pack(side=tk.LEFT)
        ttk.Button(cache_top, text="🗑  Clear cache", style="TButton",
                   command=self._on_clear_cache).pack(side=tk.RIGHT)
        ...
        for val, lbl in [("none", "None"), ("preset", "From literature"),
                         ("custom", "Enter manually")]:
            ttk.Radiobutton(mode_row, text=lbl, value=val,
                            variable=self._exp_mode, style="TRadiobutton",
                            command=self._on_exp_mode_change).pack(
                                side=tk.LEFT, padx=(0, 16))

        self._preset_frame = ttk.Frame(exp_sec, style="TFrame")
        self._preset_frame.pack(fill=tk.X, pady=(6, 0))
        ttk.Label(self._preset_frame, text="Dataset:",
                  style="Muted.TLabel").pack(side=tk.LEFT, padx=(0, 8))
        ttk.Combobox(
            self._preset_frame, textvariable=self._preset_var,
            values=list(PRESET_DATA.keys()), state="readonly",
            font=("Segoe UI", 9), width=62,
        ).pack(side=tk.LEFT)
        ...
        self._custom_text = scrolledtext.ScrolledText(
            self._custom_frame, height=6, width=40,
            font=("Consolas", 9),
            bg=th.SURFACE0, fg=th.TEXT, insertbackground=th.TEXT,
            relief="flat", highlightthickness=1,
            highlightcolor=th.BLUE, highlightbackground=th.SURFACE1,
        )
        ...
        self._run_btn = ttk.Button(
            self._inner, text="▶   Run Calculation",
        ...
        ttk.Button(
            btn_row, text="📊  View Properties",
            style="TButton", command=self._open_properties_window,
        ).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 4))
        ttk.Button(
            btn_row, text="📈  Open Plot Builder",
            style="TButton", command=self._open_plot_builder,
        ...
        ttk.Label(self._inner, textvariable=self._status_var,
                  style="Muted.TLabel", wraplength=820).pack(anchor=tk.W)

    # ── Helper ────────────────────────────────────────────────────────────────

    def _section(self, title: str) -> ttk.LabelFrame:
        return ttk.LabelFrame(self._inner, text=f"  {title}  ",
                              style="TLabelframe", padding=(10, 8))
    ...
    def _add_gas_row(self, gas: str = "CO2", frac: float = 0.5):
        ...
        ttk.Combobox(row, textvariable=gas_var, values=AVAILABLE_GASES,
                     state="readonly", width=10,
                     font=("Segoe UI", 9)).pack(side=tk.LEFT, padx=(0, 12))
        ttk.Label(row, text="Mole fraction:", style="Muted.TLabel").pack(side=tk.LEFT)
        ttk.Entry(row, textvariable=frac_var, width=10,
                  font=("Segoe UI", 10)).pack(side=tk.LEFT, padx=(4, 12))

        entry = {"gas_var": gas_var, "frac_var": frac_var, "frame": row}
        del_btn = ttk.Button(row, text="✕", style="TButton", width=3,
    ...
    def _add_gas_row_dialog(self):
        ...
        if not avail:
            messagebox.showinfo("No more gases",
                                "All available gases are already added.")
            return
    ...
    def _delete_gas_row(self, entry: dict):
        if len(self._gas_rows) <= 1:
            messagebox.showwarning("Cannot delete",
                                   "At least one gas component is required.")
            return
        entry["frame"].destroy()
    ...
    def _refresh_liquid_display(self, *_):
        ...
        self._liq_display.configure(
            text=f"H₂O: {xw:.4f}    {name}: {xp:.4f}"
        )
    ...
    def _on_run(self):
        try:
            gas_comp, liq_comp, T_range, eos_names, exp_data = \
